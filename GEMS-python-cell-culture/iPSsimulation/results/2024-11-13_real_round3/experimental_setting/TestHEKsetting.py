...
class HEKSamplingState(State):
    def task_generator(self, df: pl.DataFrame) -> TaskGroup:
        return TaskGroup(
            optimal_start_time = get_now_min_unix(),#int(optimal_time-HEK_PROCESSING_TIME["HEKSampling"]-HEK_PROCESSING_TIME["HEKGetImage"]),
            penalty_type=CyclicalRestPenaltyWithLinear(
                cycle_start_time = UNIX_2024_11_13_00_00_00_IN_JP//60 - 24 * 60,
                cycle_duration = 60*24,
                rest_time_ranges = [(0, 60*15), (60*16, 60*24)],
...
def HEKAsetting():
    ...
    new_experiment = HEKExperiment(
        shared_variable_history=shared_variable_history,
        current_state_name="HEKSamplingState",
