...
def iPSAsetting():
    ...
    28859675,iPSPassage,
    """
    shared_variable_history = pl.read_csv("iPSsimulation/results/2024-11-13_real_round3/step_00000144/experiments/IPSExperiment_iPS_Experiment_0_shared_variable_history.csv")
    new_experiment = IPSExperiment(
        shared_variable_history=shared_variable_history,
        current_state_name="MediumChange2State",
        experiment_uuid = f"iPS_Experiment_{i}"
    )
    print(f"{new_experiment=}")
    optimal_time: float = calculate_optimal_time_from_df(shared_variable_history, target_density=PASSAGE_DENSITY, density_operation_name="iPSGetImage", passage_operation_name="iPSPassage")
    print(f"passage:{optimal_time=}")
